...
$deleteTrip = function () {
    if (!$this->trip->canDelete(auth()->user())) {
    ...
    return redirect()->route('dashboard')
        ->with('status', "Trip '{$name}' has been deleted.");
};

?>
<div class="py-8 bg-bg-secondary min-h-screen font-sans text-text-main"
    x-data="{ groupTab: 'chat' }"
    x-init="if(window.Motion) { window.Motion.animate($el, { opacity: [0, 1] }, { duration: 0.4 }) }">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        <!-- Back to my trips -->
        <div class="mb-5" x-init="if(window.Motion) { window.Motion.animate($el, { x: [-10, 0], opacity: [0, 1] }, { duration: 0.4, delay: 0.05 }) }">
            <a href="{{ route('dashboard') }}" wire:navigate class="inline-flex items-center gap-2 text-xs font-bold text-text-muted hover:text-brand-neutral transition-colors group cursor-pointer">
                <i class="ph-bold ph-arrow-left transition-transform group-hover:-translate-x-0.5"></i>
        ...
        @endphp
        <div class="relative z-0 border border-border-light rounded-3xl p-6 md:p-8 mb-8 shadow-sm overflow-hidden bg-brand-neutral bg-cover bg-center"
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
                <div>
                    ...
                    <button type="button"
                        wire:click="deleteTrip"
                        wire:confirm="Delete '{{ $trip->name }}'? This will permanently remove its itinerary, polls, expenses, documents, and chat."
                        class="mt-5 inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/15 hover:bg-red-500/90 border border-white/25 text-white text-xs font-bold transition cursor-pointer">
                        <i class="ph ph-trash"></i>
                        <span>Delete trip</span>
                    </button>
                    @endif
                </div>
                <div class="bg-white/10 backdrop-blur-md border border-white/25 px-5 py-4 rounded-2xl text-center min-w-40 shadow-[0_4px_12px_rgba(0,0,0,0.1)]">
        ...
        <div class="flex flex-col lg:flex-row gap-8 items-start relative">
            ...
            <!-- LEFT PANEL: Primary Workspaces -->
            <div class="w-full lg:w-2/3">

                <!-- Workspace Tabs -->
                <div class="border-b border-border-light bg-bg-primary rounded-2xl p-1.5 border flex space-x-1 overflow-x-auto scrollbar-none">
                    @foreach ([
                ...
                <!-- Tab Views Container -->
                <div class="transition-all duration-350 mt-6">
            ...
            <!-- RIGHT PANEL: Group Hub -->
            <div class="w-full lg:w-1/3 shrink-0">
                <div class="bg-bg-primary border border-border-light rounded-2xl p-4 shadow-sm lg:sticky lg:top-24">
                    <div class="border border-border-light rounded-xl p-1 flex gap-1 bg-bg-secondary/50">
                        <button type="button"
                            x-on:click="groupTab = 'chat'"
                            class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-[11px] font-bold transition"
                            :class="groupTab === 'chat' ? 'bg-brand-neutral text-bg-primary shadow-sm' : 'text-text-muted hover:text-text-main'">
                            <i class="ph-bold ph-chats-circle text-xs"></i>
                            <span>Chat</span>
                        </button>
                        <button type="button"
                            x-on:click="groupTab = 'polls'"
                            class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-[11px] font-bold transition"
                            :class="groupTab === 'polls' ? 'bg-brand-neutral text-bg-primary shadow-sm' : 'text-text-muted hover:text-text-main'">
                            <i class="ph-bold ph-handshake text-xs"></i>
                            <span>Polls</span>
                        </button>
                        <button type="button"
                            x-on:click="groupTab = 'updates'"
                            class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-[11px] font-bold transition"
                            :class="groupTab === 'updates' ? 'bg-brand-neutral text-bg-primary shadow-sm' : 'text-text-muted hover:text-text-main'">
                    ...
                    <div class="mt-4">
                        <div x-show="groupTab === 'chat'" x-transition.opacity.duration.200ms>
                            <livewire:trip.discussion-sidebar :trip="$trip" />
                        </div>
                        <div x-show="groupTab === 'polls'" x-transition.opacity.duration.200ms style="display: none;">
                            <livewire:trip.polls-sidebar :trip="$trip" />
                        </div>
                        <div x-show="groupTab === 'updates'" x-transition.opacity.duration.200ms style="display: none;">
                            <livewire:trip.recent-updates :trip="$trip" />
                        </div>
                    </div>
                </div>
            </div>
