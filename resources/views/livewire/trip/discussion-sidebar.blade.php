...
$addComment = function () {
    // Only members who can vote (organizer/co_planner/member) can comment
...
$addReply = function ($parentId) {
    ...
    // Ensure parent comment belongs to this trip
    $parentComment = $this->trip->comments()->findOrFail($parentId);
...
$deleteComment = function ($commentId) {
    ...
    $comment = $this->trip->comments()->findOrFail($commentId);
...
<div class="bg-bg-primary border border-border-light rounded-3xl p-5 shadow-sm flex flex-col"
    ...
    <div class="flex-grow overflow-y-auto space-y-3.5 pr-1 mb-3 text-xs">
        ...
        <div class="text-center py-10">
            <i class="ph ph-chats-circle text-3xl text-text-muted/40 block mb-2"></i>
            <p class="text-text-muted text-xs">No messages yet. Start the conversation!</p>
        </div>
        @else
        ...
        <div class="space-y-1">
            <div class="flex justify-between items-start gap-1">
                <div class="flex items-center space-x-2">
                    <x-avatar :user="$comment->user" class="h-6 w-6 text-[9px]" />
                    <div>
                ...
                <div class="space-y-0.5">
                    <div class="flex justify-between items-start gap-1">
                        <div class="flex items-center space-x-1.5">
                            <x-avatar :user="$reply->user" class="h-5 w-5 text-[8px] bg-bg-secondary text-text-main border border-border-card" />
                            <div>
    ...
    <form wire:submit.prevent="addComment" class="border-t border-border-light pt-2.5 mt-auto">
        <div class="relative flex items-center">
            <textarea wire:model="new_comment_content" rows="1" placeholder="Send a message..."
