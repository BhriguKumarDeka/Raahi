...
$expenses = computed(function () {
    $tripModel = $this->trip instanceof \App\Models\Trip ? $this->trip : \App\Models\Trip::find($this->trip['id']);
    return $tripModel->expenses()->with('payer', 'splits.user')->get();
});
...
$settlements = computed(function () {
    ...
    $tripModel = $this->trip instanceof \App\Models\Trip ? $this->trip : \App\Models\Trip::find($this->trip['id']);
    $tripExpenses = $tripModel->expenses()->with('splits')->get();
...
$deleteExpense = function ($id) {
    ...
    $expense = $this->trip->expenses()->findOrFail($id);
    $expense->delete();
    $this->dispatch('trip-updated');
};
