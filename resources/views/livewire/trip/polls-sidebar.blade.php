...
$polls = computed(function () {
    return $this->trip->polls()->with('options.votes', 'votes', 'creator')->get();
});
...
<div class="bg-bg-primary border border-border-light rounded-3xl p-5 shadow-sm"
    ...
    <div class="flex justify-between items-center border-b border-border-light pb-2.5 mb-4">
        ...
        <button type="button"
            ...
            class="inline-flex items-center gap-1.5 text-[10px] font-bold text-brand-neutral hover:text-brand-hover bg-brand-neutral/5 hover:bg-brand-neutral/10 px-2.5 py-1 rounded-lg transition focus:outline-none cursor-pointer">
    ...
    <div class="text-center py-6">
        <p class="text-xs text-text-muted font-medium">No active polls.</p>
    </div>
    @else
    <div class="space-y-4 max-h-[350px] overflow-y-auto pr-1">
        ...
        <div class="border border-border-light rounded-2xl p-4 bg-bg-secondary/30 relative">
            ...
            <div class="mt-3.5 space-y-2">
                ...
                $userVote = $poll->votes->firstWhere('user_id', auth()->id());
                @endphp
                @foreach ($poll->options as $opt)
                @php
                $optVoteCount = $opt->votes->count();
                $percentage = $totalVotes > 0 ? ($optVoteCount / $totalVotes) * 100 : 0;
                ...
                <button type="button"
                    ...
                    <div class="flex justify-between items-center relative z-10 text-[11px] font-semibold">
                        ...
                        <span class="text-[10px] text-text-muted">{{ $optVoteCount }} {{ Str::plural('vote', $optVoteCount) }}</span>
                    </div>
                </button>
                @endforeach
            </div>
    ...
    <!-- POLL MODAL -->
    @if ($showPollModal)
    <div class="fixed inset-0 z-[999] flex items-center justify-center bg-black/40 backdrop-blur-sm transition-opacity">
