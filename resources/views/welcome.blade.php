<x-marketing-layout>
    ...
    <section class="relative min-h-[90vh] flex flex-col justify-center pt-24 pb-16 overflow-hidden bg-bg-secondary border-b border-border-light">
        ...
        <div class="absolute inset-0 z-0">
            <img src="https://images.pexels.com/photos/34244310/pexels-photo-34244310.jpeg/" 
                 ...
                 class="w-full h-full object-fit filter contrast-1.1" />
        ...
        <div class="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white mt-12">
            ...
            <!-- Streamlined Search Widget with Auto-Typing and Glow Focus -->
            <div class="search-widget mt-12 max-w-2xl mx-auto relative group">
                <!-- Focus glow background -->
                <div class="absolute -inset-0.5 bg-linear-to-r from-emerald-500 to-teal-500 rounded-full blur opacity-0 group-focus-within:opacity-40 transition duration-500"></div>
                
                <div class="relative flex items-center justify-between bg-white rounded-full p-1.5 shadow-[0_20px_50px_rgba(26,59,43,0.12)] border border-border-card text-text-main">
                    <!-- Search Icon (Phosphor) -->
                    <div class="pl-5 text-text-muted">
                        <i class="ph ph-magnifying-glass text-lg"></i>
                    </div>
                    <!-- Single input with auto-typing placeholder -->
    ...
    <section class="py-12 bg-bg-primary">
        <div class="max-w-5xl mx-auto px-4 text-center">
            <div class="flex flex-wrap items-center justify-center gap-8 md:gap-16 opacity-40 grayscale contrast-125">
                <x-simpleicon-airbnb class="h-7 w-auto text-text-muted fill-current hover:text-brand-neutral transition-colors duration-300" />
                <x-simpleicon-expedia class="h-7 w-auto text-text-muted fill-current hover:text-brand-neutral transition-colors duration-300" />
                <x-simpleicon-uber class="h-7 w-auto text-text-muted fill-current hover:text-brand-neutral transition-colors duration-300" />
                <x-simpleicon-bookingdotcom class="h-7 w-auto text-text-muted fill-current hover:text-brand-neutral transition-colors duration-300" />
                <x-simpleicon-tripadvisor class="h-7 w-auto text-text-muted fill-current hover:text-brand-neutral transition-colors duration-300" />
            </div>
        </div>
    </section>
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col lg:flex-row items-start gap-12">
                ...
                <div class="w-full lg:w-2/5 flex flex-col justify-between">
                    <div>
                        <h2 class="text-3xl sm:text-4xl font-extrabold tracking-tight leading-tight text-text-main text-left">
    ...
    <section id="discover" class="py-24 bg-bg-secondary border-t border-b border-border-light" x-data="{ currentTab: 'india' }">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <!-- Heading -->
            <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-10 text-center md:text-left">
                ...
                <div class="bg-bg-primary border border-border-card p-1 rounded-full flex gap-1 shadow-sm">
                    <button @click="currentTab = 'india'" :class="currentTab === 'india' ? 'bg-brand-neutral text-bg-primary shadow-xs' : 'text-text-muted hover:text-text-main'" class="px-6 py-2 rounded-full font-bold text-xs uppercase tracking-wider transition-all duration-200 cursor-pointer">
                        India
                    </button>
                    <button @click="currentTab = 'abroad'" :class="currentTab === 'abroad' ? 'bg-brand-neutral text-bg-primary shadow-xs' : 'text-text-muted hover:text-text-main'" class="px-6 py-2 rounded-full font-bold text-xs uppercase tracking-wider transition-all duration-200 cursor-pointer">
                        Abroad
                    </button>
                </div>
            </div>
            ...
            <!-- India Section -->
            <div x-show="currentTab === 'india'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <x-destination-card 
            ...
            <!-- Abroad Section -->
            <div x-show="currentTab === 'abroad'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="grid grid-cols-1 md:grid-cols-3 gap-6" style="display: none;">
                <x-destination-card 
                    title="Uluwatu Beach House"
                    location="Badung Regency, Bali"
                    image="https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?auto=format&fit=crop&w=600&q=80"
                    duration="6 Days"
                    type="Open Trip"
                    dates="12-18 Aug"
                    accommodation="2N Seaside Villa, 3N Resort"
                    transport="Private Van & Local Driver"
                    meals="Daily Breakfast & Dinner"
                    price="$1,150"
                    ctaUrl="/register"
                />
                ...
                />
                <x-destination-card 
                    title="Patagonia Alpine Yurt"
                    location="West Patagonia, Chile"
                    image="https://images.unsplash.com/photo-1527004013197-933c4bb611b3?auto=format&fit=crop&w=600&q=80"
                    duration="8 Days"
                    type="Expedition"
                    dates="20-28 Dec"
                    accommodation="Glamping Yurts & Chalets"
                    transport="4x4 Offroad Transfers"
                    meals="All Camp Meals & Cook"
                    price="$3,100"
                    ctaUrl="/register"
                />
            </div>
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
                ...
                <div class="stat-item">
                    ...
                    <p class="text-[10px] font-bold text-text-muted uppercase tracking-widest mt-2">Expenses Netted & Split</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section id="features" class="py-24 bg-bg-secondary border-t border-b border-border-light">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                ...
                <div class="md:col-span-2 bg-bg-primary border border-border-card rounded-lg p-6 relative overflow-hidden group flex flex-col justify-between min-h-90 transition-all duration-300">
                    <div class="max-w-md z-10 text-left">
                        <h3 class="text-xl font-bold text-text-main mb-2">Make a Trip</h3>
                        <p class="text-xs text-text-muted leading-relaxed">
                            Design day-by-day itineraries with a beautiful interactive scrolling timeline. Connect flights, hotels, and custom spots into one unified view.
                        </p>
                    </div>
                    
                    <!-- Custom UI Illustration for Make a Trip (Timeline) -->
                    <div class="relative mt-6 h-40 rounded-2xl border border-border-light bg-bg-secondary overflow-hidden p-4 select-none">
                        <div class="space-y-4">
                            <div class="flex items-center gap-3">
                                <span class="px-2.5 py-1 bg-brand-neutral text-bg-primary text-[9px] font-bold rounded-full">Day 1</span>
                                <div class="h-px grow bg-border-card"></div>
                            </div>
                            <div class="pl-4 border-l border-brand-neutral/20 ml-5 space-y-3">
                                <div class="bg-white border border-border-card rounded-xl p-3 shadow-xs max-w-sm flex items-center justify-between">
                                    ...
                                    <span class="text-[9px] px-2 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-md font-bold">Confirmed</span>
                                </div>
                                <div class="bg-white border border-border-card rounded-xl p-3 shadow-xs max-w-sm flex items-center justify-between">
                                    <div class="flex items-center gap-2.5">
                                        ...
                                        <div class="text-left">
                                            ...
                                            <p class="text-[9px] text-text-muted">Traditional Suite • Check-in 03:00 PM</p>
                                        </div>
                                    </div>
                                    <span class="text-[9px] px-2 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-md font-bold">Confirmed</span>
                                </div>
                        ...
                        <div class="absolute -right-10 -bottom-10 w-40 h-40 bg-brand-neutral/5 rounded-full blur-2xl pointer-events-none"></div>
                    </div>
                </div>
                
                <!-- Cell 2: Group Collab (Spans 1 col) -->
                <div class="bg-bg-primary border border-border-card rounded-lg p-6 relative overflow-hidden group flex flex-col justify-between min-h-90 transition-all duration-300">
                    <div class="z-10 text-left">
                        <h3 class="text-xl font-bold text-text-main mb-2">Group Collab</h3>
                        <p class="text-xs text-text-muted leading-relaxed">
                            Work live with your friends. Add members with read/write access and plan together with instantaneous sync.
                        </p>
                    </div>
                    
                    <!-- Custom UI Illustration for Group Collab (Active planners & Chat bubbles) -->
                    <div class="relative mt-6 h-40 rounded-2xl border border-border-light bg-bg-secondary overflow-hidden p-4 select-none flex flex-col justify-end gap-3 font-left">
                        <div class="flex items-center gap-2">
                            <div class="flex -space-x-2">
                                <span class="w-7 h-7 rounded-full bg-emerald-700 text-white flex items-center justify-center text-[10px] font-bold border-2 border-bg-secondary">RD</span>
                                <span class="w-7 h-7 rounded-full bg-amber-600 text-white flex items-center justify-center text-[10px] font-bold border-2 border-bg-secondary">AK</span>
                                <span class="w-7 h-7 rounded-full bg-sky-600 text-white flex items-center justify-center text-[10px] font-bold border-2 border-bg-secondary">MS</span>
                            </div>
                            <span class="text-[9px] font-semibold text-text-muted">3 online now</span>
                        </div>
                        
                        <div class="space-y-2">
                            <div class="bg-white border border-border-card rounded-2xl rounded-bl-none p-2.5 shadow-xs max-w-[85%] self-start text-left">
                            ...
                            <div class="bg-brand-neutral text-bg-primary rounded-2xl rounded-br-none p-2.5 shadow-xs max-w-[85%] self-end ml-auto text-left">
                ...
                <div class="bg-bg-primary border border-border-card rounded-lg p-6 relative overflow-hidden group flex flex-col justify-between min-h-90 transition-all duration-300">
                    <div class="z-10 text-left">
                        ...
                        <p class="text-xs text-text-muted leading-relaxed">
                            Can't agree on lodging or hikes? Create elegant choice polls and let the majority vote decide the plan.
                        </p>
                    </div>
                    
                    <!-- Custom UI Illustration for Poll Request -->
                    <div class="relative mt-6 h-40 rounded-2xl border border-border-light bg-bg-secondary overflow-hidden p-4 select-none flex flex-col justify-center gap-3 text-left">
                ...
                <div class="md:col-span-2 bg-bg-primary border border-border-card rounded-lg p-6 relative overflow-hidden group flex flex-col justify-between min-h-90 transition-all duration-300">
                    <div class="max-w-md z-10 text-left">
                        ...
                        <p class="text-xs text-text-muted leading-relaxed">
                            Log expenses in any currency and split them with customizable ratios. View exact balances showing who owes who.
                        </p>
                    </div>
                    
                    <!-- Custom UI Illustration for Budget Tracking -->
                    <div class="relative mt-6 h-40 rounded-2xl border border-border-light bg-bg-secondary overflow-hidden p-4 select-none flex flex-col justify-between text-left">
                        <div class="flex items-center justify-between border-b border-border-light pb-2">
                            ...
                            <span class="text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100">Total: ₹84,500</span>
                        </div>
                        <div class="space-y-2 mt-2 grow overflow-hidden">
                            <div class="flex justify-between items-center text-[9px] bg-white p-2 border border-border-card rounded-lg">
                                <div class="flex items-center gap-2">
                                    <i class="ph ph-bowl-food text-text-muted"></i>
                                    <span class="text-text-main">Sushi Dinner (Kyoto)</span>
                                </div>
                                <span class="font-bold text-text-main">₹12,400 <span class="text-[8px] text-text-muted font-normal">by Rohan</span></span>
                            </div>
                            <div class="flex justify-between items-center text-[9px] bg-white p-2 border border-border-card rounded-lg">
                                <div class="flex items-center gap-2">
                                ...
                                <span class="font-bold text-text-main">₹36,000 <span class="text-[8px] text-text-muted font-normal">by You</span></span>
                            </div>
                        </div>
                        <div class="bg-brand-neutral/5 border border-brand-neutral/10 rounded-xl p-2.5 mt-2 flex justify-between items-center">
                            <span class="text-[9px] font-bold text-brand-neutral flex items-center gap-1">
                                <i class="ph ph-hand-coins"></i>
                                <span>Rohan owes you ₹5,800</span>
                            </span>
                            <button class="text-[8px] font-bold uppercase bg-brand-neutral text-bg-primary px-2.5 py-1 rounded-md cursor-pointer hover:bg-brand-hover">Settle Up</button>
                        </div>
                    </div>
                </div>
                
                <!-- Cell 5: Clone Popular Itinerary -->
                <div class="md:col-span-3 bg-bg-primary border border-border-card rounded-lg p-6 relative overflow-hidden group flex flex-col md:flex-row items-center justify-between transition-all duration-300">
                    <div class="max-w-md z-10 text-left">
                        <h3 class="text-xl font-bold text-text-main mb-2">Clone Itinerary</h3>
                        <p class="text-xs text-text-muted leading-relaxed">
                            Don't start from scratch. Browse world-class itineraries created by seasoned backpackers and explorers, and clone them into your group workspace with a single click.
                        </p>
                    </div>
